...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    ...
    BEGIN
        IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'developer') THEN
            CREATE ROLE developer WITH LOGIN CREATEDB PASSWORD '${DEVELOPER_PASSWORD}';
        END IF;
    END \$\$;
EOSQL
...
psql -v ON_ERROR_STOP=1 --username "developer" --dbname "app" <<-EOSQL
...
echo "   - Roles:     prefect (superuser), developer (app admin), readonly"
echo "   - Databases: prefect (Prefect Server), app (flight data, owned by developer)"